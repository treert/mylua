...
static void suffixedexp (LexState *ls, expdesc *v) {
  ...
  for (;;) {
    switch (ls->t.token) {
      ...
      case '(': case TK_STRING: case '{': case '$': {  /* funcargs */ // add@om $
...
static BinOpr getbinopr (int op) {
  switch (op) {
    ...
    case TK_QQUESTION: return OPR_QQ;// add@om
...
#define UNARY_PRIORITY	22  /* mod@om priority for unary operators */
...
static void gotostat (LexState *ls) {
  FuncState *fs = ls->fs;
  int line = ls->linenumber;
  TString *name = str_check_name_and_keyword(ls);  /* label's name */ // mod@om
...
static void repeatstat (LexState *ls, int line) {
  ...
  insertcontinelabel(ls);// add@om continue label, 很大的可能不允许跳到这儿来，主要是变量的作用域问题。
...
static void statement (LexState *ls) {
  ...
  switch (ls->t.token) {
    ...
    case TK_DBCOLON: {  /* stat -> label */
      ...
      labelstat(ls, str_check_name_and_keyword(ls), line);// mod@om
      break;
    }
